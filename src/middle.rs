    ...
    match validate_token(req.to_str().unwrap()).await {
        Ok(res) => {
            if res {
                ...
                let user_local_id = match auth.get_user_info(req.to_str().unwrap()).await {
                    Ok(user) => user.local_id,
                    Err(_) => return Err(actix_web::error::ErrorInternalServerError("sushi!")),
                };
                ...
                match search_user_from_db(&mut conn, user_local_id.clone()) {
                    ...
                    false => {
                        ...
                        let user_info = match auth.get_user_info(req.to_str().unwrap()).await {
                            Ok(user) => user,
                            Err(_) => return Err(actix_web::error::ErrorInternalServerError("sushi!")),
                        };
